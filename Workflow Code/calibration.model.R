...
conf <- rowSums(Y[,c('PINUSX','PICEAX','TSUGAX','LARIXPSEU')])
grass <- Y[,c('prairie')]#,'other herbs','CYPERACE'
out <- rowSums(Y[,c('other herbs','CYPERACE')])
other <- rowSums(Y) - conf - grass - out
Y <- cbind(grass,conf,other)

trees.names <- c("PINUSX","ALNUSX","JUGLANSX","ACERX","CUPRESSA","FRAXINUX","FAGUS","CYPERACE","LARIXPSEU","TSUGAX","QUERCUS","TILIA","BETULA","PICEAX","OSTRYCAR","ULMUS","ABIES","POPULUS","other trees")
trees <- rowSums(Y[,trees.names])
other <- rowSums(Y) - trees - grass
Y <- cbind(trees,grass,other)
...
for(i in 1:ncol(samples.pred)){
  ...
  points(samples.pred[,i],typ = 'l',col='blue')
  abline(h=biomass[i],col='red')
  hist(samples.pred[,i],freq = FALSE, main = i)
  lines(density(samples.pred[,i]),col = 'blue')
}
dev.off()

pdf(paste0("pred_validation_tree_v_no_tree",Sys.Date(),".pdf"))
...
abline(a=0,b=1)
#abline(lm(biomass~colMeans(samples.pred[100:nrow(samples.pred),grep('b',colnames(samples.pred))])),lty=2)
